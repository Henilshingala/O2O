...
export default function CreateBidScreen() {
  ...
  const handlePickImage = async () => {
    ...
    try {
      ...
      console.error("Bid product image upload error:", e);
      setErrors((prev) => ({ ...prev, productImage: "Upload failed: " + (e?.message || "") }));
      setProductImage("");
    } finally {
      setUploading(false);
    }
  ...
  const handleNext = () => {
    ...
    if (!form.description.trim()) e.description = "Please add details (location, timeline, etc.)";
    if (uploading) e.productImage = "Please wait for image upload";
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior="height">
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={[styles.imageBox, { backgroundColor: colors.muted, borderColor: previewUri ? colors.primary : colors.border }]} onPress={handlePickImage}>
          ...
          {errors.productImage && <Text style={{ color: colors.destructive, fontSize: 12, marginTop: 4 }}>{errors.productImage}</Text>}
        </TouchableOpacity>

        <AppInput label="Product Name" value={form.productName} onChangeText={set("productName")} placeholder="What do you want to buy?" error={errors.productName} editable={!uploading} />
        <AppInput label="Quantity Needed" value={form.quantity} onChangeText={set("quantity")} placeholder="How many units?" keyboardType="numeric" error={errors.quantity} editable={!uploading} />
        <AppInput label="Budget Per Product (₹)" value={form.budget} onChangeText={set("budget")} placeholder="Your max price per unit" keyboardType="numeric" error={errors.budget} editable={!uploading} />
        <AppInput label="Bid Description" value={form.description} onChangeText={set("description")} placeholder="Add delivery requirements, location, timeline..." multiline style={{ height: 90, textAlignVertical: "top", paddingTop: 10 }} error={errors.description} editable={!uploading} />
