...
interface DataContextType {
  ...
  markBidSeen: (bidId: string) => Promise<void>;
  markBidRead: (bidId: string) => Promise<void>;
  markOfferSeen: (bidId: string, offerId: string) => Promise<void>;
  markOfferRead: (bidId: string, offerId: string) => Promise<void>;
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: counts = { notifications: 0, wishlist: 0, orders: 0, requests: 0, channels: 0, bids: 0, messages: 0 }, isLoading: loadingCounts } = useQuery<Counts>({
    queryKey: ["counts"],
  ...
  const invalidate = {
    ...
    counts: () => queryClient.invalidateQueries({ queryKey: ["counts"] }),
  };
  ...
  const markBidSeen = useCallback((bidId: string) => {
    return customFetch(`/api/data/bids/${bidId}/seen`, { method: "POST" })
      .then(() => {
        invalidate.bids();
        invalidate.counts();
      });
  ...
  const markBidRead = useCallback((bidId: string) => {
    return customFetch(`/api/data/bids/${bidId}/read`, { method: "POST" })
      .then(() => {
        invalidate.bids();
        invalidate.counts();
      });
  ...
  const markOfferSeen = useCallback((bidId: string, offerId: string) => {
    return customFetch(`/api/data/bids/${bidId}/offers/${offerId}/seen`, { method: "POST" })
      .then(() => {
        invalidate.bids();
        invalidate.counts();
      });
  ...
  const markOfferRead = useCallback((bidId: string, offerId: string) => {
    return customFetch(`/api/data/bids/${bidId}/offers/${offerId}/read`, { method: "POST" })
      .then(() => {
        invalidate.bids();
        invalidate.counts();
      });
  ...
  const submitOffer = useCallback((bidId: string, offer: Omit<BidOffer, "id" | "timestamp">) => {
    return customFetch(`/api/data/bids/${bidId}/offers`, { method: "POST", body: JSON.stringify(offer) })
      .then((updated: unknown) => {
        const typedUpdated = updated as BidOffer;
