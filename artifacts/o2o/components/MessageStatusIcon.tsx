...
export function MessageStatusIcon({ status, isMine }: MessageStatusIconProps) {
  ...
  if (status === "seen") {
    // Double blue/teal checks — like WhatsApp "seen"
    return <Feather name="check-circle" size={12} color="#34D399" style={styles.icon} />;
  }
  if (status === "delivered") {
    return (
      <Feather
        name="check-circle"
        size={12}
        color={isMine ? "rgba(255,255,255,0.85)" : colors.success}
        style={styles.icon}
      />
    );
  }
