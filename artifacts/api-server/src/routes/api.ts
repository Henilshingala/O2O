...
router.post("/channels/:id/messages", validateBody(sendMessageSchema), async (req: AuthRequest, res) => {
  try {
    const channelId = req.params.id as string;
...
router.get("/wishlist", async (req: AuthRequest, res) => {
  try {
    ...
    const total = countResult[0]?.count ?? 0;
    const rows = await db.select().from(schema.wishlist).where(eq(schema.wishlist.userId, userId)).limit(limit).offset(offset);
    const productIds = rows.map((row) => row.productId);
    const products = productIds.length > 0 ? await db.select().from(schema.products).where(inArray(schema.products.id, productIds)) : [];
    ...
    const list = rows.map((row) => {
      ...
      const channel = product ? channelMap[product.channelId] : undefined;
      return {
...
router.get("/counts", async (req: AuthRequest, res) => {
  try {
    const userId = req.user!.userId;
    const [notifResult] = await db.select({ count: count() }).from(schema.notifications).where(and(eq(schema.notifications.userId, userId), eq(schema.notifications.isRead, false)));
    const [wishlistResult] = await db.select({ count: count() }).from(schema.wishlist).where(eq(schema.wishlist.userId, userId));
    const [pendingOrderResult] = await db.select({ count: count() }).from(schema.orders).where(and(eq(schema.orders.buyerId, userId), eq(schema.orders.status, "pending")));
    const [requestResult] = await db.select({ count: count() }).from(schema.friendsContacts).where(and(eq(schema.friendsContacts.contactId, userId), eq(schema.friendsContacts.status, "pending")));
    const ownedChannels = await db.select({ id: schema.channels.id }).from(schema.channels).where(eq(schema.channels.ownerId, userId));
    const followedChannels = await db.select({ count: count() }).from(schema.channelFollowers).where(eq(schema.channelFollowers.userId, userId));
    const sellerChannelIds = ownedChannels.map((c) => c.id);
    ...
    return res.json({
      ...
      orders: pendingOrderResult.count ?? 0,
...
router.post("/groups", validateBody(createGroupSchema), async (req: AuthRequest, res) => {
  try {
    const id = genId("grp");
...
router.post("/bids", validateBody(createBidSchema), async (req: AuthRequest, res) => {
  try {
    ...
    emitToUser(req.user!.userId, "bid_created", newBid);

    if (newBid.allSellers) {
      getIo()?.emit("bid_received", newBid);
    } else if (newBid.selectedSellers.length > 0) {
      const channels = await db.select({ id: schema.channels.id, ownerId: schema.channels.ownerId }).from(schema.channels).where(inArray(schema.channels.id, newBid.selectedSellers));
...
router.post("/bids/:id/seen", async (req: AuthRequest, res) => {
  try {
    const bidId = req.params.id as string;
    const userId = req.user!.userId;
    ...
    const seenBy = Array.isArray(bid.seenBy) ? bid.seenBy : [];
    if (!seenBy.includes(userId)) {
      await db.update(schema.bids).set({ seenBy: [...seenBy, userId] }).where(eq(schema.bids.id, bidId));
      emitToBid(bidId, "bid:seen", { bidId, userId });
    }
    return res.json({ success: true, bidId, seenBy: [...seenBy, userId] });
  } catch (error) { return res.status(500).json({ error: "Server error" }); }
});

router.post("/bids/:id/read", async (req: AuthRequest, res) => {
  try {
    const bidId = req.params.id as string;
    const userId = req.user!.userId;
    const bidRows = await db.select().from(schema.bids).where(eq(schema.bids.id, bidId)).limit(1);
    const bid = bidRows[0];
    if (!bid) return res.status(404).json({ error: "Bid not found" });
    const readBy = Array.isArray(bid.readBy) ? bid.readBy : [];
    if (!readBy.includes(userId)) {
      await db.update(schema.bids).set({ readBy: [...readBy, userId] }).where(eq(schema.bids.id, bidId));
      emitToBid(bidId, "bid:read", { bidId, userId });
    }
    return res.json({ success: true, bidId, readBy: [...readBy, userId] });
  } catch (error) { return res.status(500).json({ error: "Server error" }); }
});

router.post("/bids/:id/offers/:offerId/seen", async (req: AuthRequest, res) => {
  try {
    const bidId = req.params.id as string;
    const offerId = req.params.offerId as string;
    const userId = req.user!.userId;
    const offers = await db.select().from(schema.bidOffers).where(eq(schema.bidOffers.id, offerId)).limit(1);
    const offer = offers[0];
    if (!offer) return res.status(404).json({ error: "Offer not found" });
    const seenBy = Array.isArray(offer.seenBy) ? offer.seenBy : [];
    if (!seenBy.includes(userId)) {
      await db.update(schema.bidOffers).set({ seenBy: [...seenBy, userId] }).where(eq(schema.bidOffers.id, offerId));
      emitToBid(bidId, "bid:offer:seen", { bidId, offerId, userId });
    }
    return res.json({ success: true, offerId, seenBy: [...seenBy, userId] });
  } catch (error) { return res.status(500).json({ error: "Server error" }); }
});

router.post("/bids/:id/offers/:offerId/read", async (req: AuthRequest, res) => {
  try {
    const bidId = req.params.id as string;
    const offerId = req.params.offerId as string;
    const userId = req.user!.userId;
    ...
    const offer = offers[0];
    if (!offer) return res.status(404).json({ error: "Offer not found" });
    const readBy = Array.isArray(offer.readBy) ? offer.readBy : [];
    if (!readBy.includes(userId)) {
      await db.update(schema.bidOffers).set({ readBy: [...readBy, userId] }).where(eq(schema.bidOffers.id, offerId));
      emitToBid(bidId, "bid:offer:read", { bidId, offerId, userId });
    }
    return res.json({ success: true, offerId, readBy: [...readBy, userId] });
  } catch (error) { return res.status(500).json({ error: "Server error" }); }
});
...
router.post("/orders", validateBody(createOrderSchema), async (req: AuthRequest, res) => {
  try {
    const id = genId("ord");
...
router.post("/orders/:id/messages", validateBody(sendMessageSchema), async (req: AuthRequest, res) => {
  try {
